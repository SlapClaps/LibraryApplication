	...
	namespace net
	{
		...
		public:
				...
				: m_asioContext(asioContext), m_socket(std::move(socket)), m_qMessagesIn(qIn)
			{
				...
				{
					// Construct is Server -> Client, construct random data for the client
		...
		private:
			...
			// "Encrypt" the handshake validation data
			uint64_t scramble(uint64_t nInput)
			{
				uint64_t out = nInput ^ 0xC0DEF0FAF0;
				out = (out & 0xC0D3F0CAD0) >> 4 | (out & 0xBAFAC0CAC0) << 4;
				return out ^ 0xAC3BACAD01234567;
			}
